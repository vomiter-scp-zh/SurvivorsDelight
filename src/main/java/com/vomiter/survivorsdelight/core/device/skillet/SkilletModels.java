...
public class SkilletModels {
    ...
    public static void onModelRegister(ModelEvent.RegisterAdditional event) {
        for (SkilletMaterial m : SkilletMaterial.values()){
            event.register(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(SurvivorsDelight.MODID, "skillet/" + m.material +"_cooking"), "inventory"));
        }
        event.register(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(SurvivorsDelight.MODID, "skillet/" + "farmer" +"_cooking"), "inventory"));
    }

    public static void makeModel(String name, ModelEvent.ModifyBakingResult event){
        Map<ResourceLocation, BakedModel> modelRegistry = event.getModels();
        ModelResourceLocation skilletLocation = new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(SurvivorsDelight.MODID, "skillet/" + name), "inventory");
        BakedModel skilletModel = modelRegistry.get(skilletLocation);
        ModelResourceLocation skilletCookingLocation = new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(SurvivorsDelight.MODID, "skillet/" + name +"_cooking"), "inventory");
