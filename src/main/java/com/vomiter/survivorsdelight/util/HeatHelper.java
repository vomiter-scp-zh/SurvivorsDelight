...
public class HeatHelper {
    ...
    public static float getTemperature(BlockPos pos, LevelReader level, GetterType getterType){
        ...
        if(isBlock && blockState.is(SDBlockTags.HEAT_TO_BLOCK_BLACKLIST)){
            return 0;
        }

        if(isInHand && blockState.is(SDBlockTags.HEAT_TO_IN_HAND_BLACKLIST)){
            return 0;
        }
        ...
        if(blockState.is(SDBlockTags.STATIC_HEAT_MODERATE)){
            return 500f;
        } else if (blockState.is(SDBlockTags.STATIC_HEAT_HIGH)) {
            return 1500f;
        } else if (blockState.is(SDBlockTags.STATIC_HEAT_LOW)){
            return 250f;
        }
        return 0;
    }
