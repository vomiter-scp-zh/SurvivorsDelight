package com.vomiter.survivorsdelight.content.device.skillet.itemcooking;
...
public class SkilletItemCookingData implements ISkilletItemCookingData {
    ...
    public CompoundTag save() {
        ...
        if (!cooking.isEmpty()) tag.put("Cooking", cooking.save(new CompoundTag()));
        tag.putFloat("TargetTemp", targetTemperature);
        tag.putInt("Hand", hand.ordinal());
        return tag;
    }
    public void load(CompoundTag tag) {
        this.cooking = tag.contains("Cooking") ? ItemStack.of(tag.getCompound("Cooking")) : ItemStack.EMPTY;
